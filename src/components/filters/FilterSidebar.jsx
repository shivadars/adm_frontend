...
export const FilterSidebar = ({ selectedCategory, setSelectedCategory }) => {
  const categories = ['All', 'Male', 'Female', 'New Collections', 'Bandana', 'Customization'];
        ...
        <div className="space-y-3">
          {categories.map(cat => (
            <label key={cat} className="flex items-center space-x-3 cursor-pointer group">
              <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedCategory === cat ? 'bg-primary-dark border-primary-dark' : 'border-brand-border group-hover:border-primary-dark'}`}>
              ...
              <span className={`text-brand-dark/80 transition-colors ${selectedCategory === cat ? 'font-bold text-brand-dark' : 'group-hover:text-primary-dark'}`}>{cat}</span>
